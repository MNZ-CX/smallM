...
| 🗒 **多条目清单** | 逐条独立卡片：复选框 + 可点的时间胶囊 + ✎ + 🗑；勾选自动划线沉底，删除只影响该条 |
| ✏️ **随时可改** | 已添加的日程**双击（或点 ✎）就地改内容**，点时间胶囊补/改/取消提醒时间，右键还能复制、移到今天/明天、标记完成、删除 |
| 🐞 **问题收集页** | 顶栏 🐞 切到反馈页：随手记下发现的问题，勾选标记已修复；攒够了一次性「复制待修复」成编号清单，直接发给 AI 批量修 |
...
| **修改已有日程** | **双击这一条**（或点 ✎）就地改内容 → 回车保存 / Esc 取消 |
| **给已有日程设/改时间** | 点它前面的**时间胶囊**（没时间的显示「＋时间」）→ 选时间或「不提醒」 |
| 日程右键菜单 | 改内容 / 设置提醒时间 / 取消提醒 / 复制内容 / 移到今天 / 移到明天 / 标记完成 / 删除 |
| 删除待办 | 点该条右侧 🗑（**秒删**，底部浮出撤销条，3 秒内可撤回） |
| 看历史 | 点 📅 展开月历 → 点任意日期，下方清单切换为该日；悬停日期浮出当天内容 |
| 导出当天 | 点面板右上「导出」→ 编号列表进入剪贴板，直接粘贴 |
| **记录问题 / 反馈** | 顶栏 🐞（或托盘菜单「问题收集页」）→ 输入现象回车记录；勾选 = 已修复 |
| **批量修问题** | 反馈页点「复制待修复」→ 编号清单进剪贴板，粘给 AI 让它按编号一次改完；「清理已修复」清空已修好的（可撤销） |
...
**产物**：`dist\smallM.exe`（当前版本约 **36 MB**，单文件、免安装 Python）
...
- 数据文件为 `memos.json`，内含 `version` 字段（当前 `7`）
- 新版本**向下兼容**旧数据结构：早期的 `todos` 字段会自动迁移为 `schedules`，缺失字段（如 v1.1.0 新增的 `feedback`）自动补默认值
...
**Q：日程建好之后还能改吗？**
A：能，四条路都通：① **双击这一条**就地改内容（回车保存、Esc 取消）；② 点卡片上的 **✎**；③ 点它前面的**时间胶囊**补/改/取消提醒时间；④ **右键这一条**打开菜单：改内容 / 设置提醒时间 / 取消提醒 / 复制内容 / 移到今天 / 移到明天 / 标记完成 / 删除。

**Q：输入框里的字太长会看不见开头吗？**
A：不会。日程输入框改成**多行自适应**：文字自动换行、输入框随内容长高（最多约 4 行），整段内容始终看得见；超过上限后才会内部滚动。回车提交、`Shift+回车` 换行、`Esc` 清空。

**Q：问题收集页是干嘛的？**
A：顶栏 🐞 切过去，把这段时间遇到的小毛病随手记一条（现象 / 怎么复现 / 期望是什么），修好了勾一下变成「已修复」。攒够了一次性点「复制待修复」，会生成带编号的清单进剪贴板，直接粘给 AI 就能按编号批量修。「清理已修复」清掉已修好的记录（3 秒内可撤销）。
