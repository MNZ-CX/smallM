...
| ⏰ **日程 + 到点提醒** | 一行速记 `15:00 部门例会`；到点弹出**全屏居中覆盖提醒**（知道了 / 延后 10 分钟 / 标记完成）+ 系统提示音 |
...
> 只依赖 **PyQt6**，其余全部使用 Python 标准库与 Win32 原生能力（全局热键、系统通知），常驻内存约 70 MB。
...
| 新建日程 | 在输入框写 `15:00 部门例会` / `明天 10:00 评审` / `9-15 09:30 上线`，回车或点「添加」 |
